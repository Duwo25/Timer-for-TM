...
$(document).on('pagebeforeshow','#page-import-agenda', function(event){ 
    ...
    if (easySpeakServer == "no-easy-Speak") {
        $("#page-import-agenda #gotoEasyspeakAgendaDetail").addClass("ui-state-disabled");
        
    } else {
        $("#page-import-agenda #gotoEasyspeakAgendaDetail").removeClass("ui-state-disabled");
    }
...
function gotoEasyspeakAgendaDetail() {
    ...
    if (easySpeakServer != "no-easy-Speak") {
        /* Open page */
        if (isRunningAsApp()) {
            cordova.InAppBrowser.open('https://' + easySpeakServer + '/view_meeting.php?t=next&pr=2', '_blank', 'location=yes');
        } else {
            window.open('https://' + easySpeakServer + '/view_meeting.php?t=next&pr=2','_system','location=yes');
        }
    }
}
...
function startStopTimer() {
    ...
    if (timerEnabled == true) {
        ...
        //$('#page-timer a[role="button"][data-rel="back"]').prop( "disabled", false );
        ...
        $('#page-timer #btn-quick-timer').removeClass("ui-state-disabled"); // Quick Timer button
        ...
        $('#page-timer #btn-quick-timer').addClass("ui-state-disabled"); // Quick Timer button
...
function showTimerCards() {
    $.mobile.changePage( "#page-dynamic-card", { transition: "slide" });
}
...
function updateQuickTimer() {
    ...
    newQuickTimerValue (Math.round((quickTimerTotalMilliseconds + quickTimerMilliseconds) / 1000));
}

function setQuickTimerLimit(redTime) {
    quickTimerRed = redTime;
    if (quickTimerEnabled == true) {updateQuickTimer();}
}
function startStopQuickTimer() {
    ...
    if (quickTimerEnabled == true) {
        ...
        quickTimerTotalMilliseconds = quickTimerTotalMilliseconds + quickTimerMilliseconds;
        // jQuery Mobile specific
        ...
        $('#page-quick-timer a[role="button"][data-rel="back"]').removeClass("ui-state-disabled"); // Back button
        ...
        $('#page-quick-timer a[role="button"][data-rel="back"]').addClass("ui-state-disabled"); // Back button
...
$( document ).on( "swipeleft", ".page-timer-card[data-role=page]", function( event ) {
	var pageRef = $("#btn-next", this).attr("href");
	if (pageRef != null) {
		// Using changePage instead of the newer navigate method,
		// because the reverse option hasn't been properly implemented yet
		$.mobile.changePage( pageRef, { transition: "slide" });
	}
});

$( document ).on( "swiperight", ".page-timer-card[data-role=page]", function( event ) {
	console.log($("#btn-prev", this).attr("href"));
	var pageRef = $("#btn-prev", this).attr("href");
	if (pageRef != null) {
		// Using changePage instead of the newer navigate method,
		// because the reverse option hasn't been properly implemented yet
		$.mobile.changePage( pageRef, { transition: "slide", reverse: true });
	}
});
